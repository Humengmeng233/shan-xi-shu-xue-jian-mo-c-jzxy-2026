...
def read_year_actuals_and_tariff() -> pd.DataFrame:
    ...
    tariff = wide_sheet_to_long(RAW_DIR / "附件4.xlsx", "Sheet1", "electricity_price_yuan_per_kwh")
...
def check_frame(
    frame: pd.DataFrame, key_columns: list[str], value_columns: list[str]
) -> dict[str, object]:
...
def main() -> None:
    ...
    typical_consistency = {
        ...
        "pv_max_abs_difference_from_annual_interval_mean_kw": float(
            np.max(
                np.abs(
                    typical["pv_forecast_kw"].to_numpy() - annual_means["pv_actual_kw"].to_numpy()
                )
            )
        ),
    }
    ...
    quality = {
        "source_scope": {
            ...
            "excluded_from_inputs": "data/templates 与 data/interim/templates 为结果模板或既有结果，不作为原始输入。",

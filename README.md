...
要求 Python 3.12 或更高版本。依赖由 `uv.lock` 锁定，无需安装项目包或手动设置模块搜索路径。在项目根目录执行：
...
通用模块：[paths.py](src/paths.py) 定义路径，[model_config.py](src/model_config.py) 定义参数，[preprocessing.py](src/preprocessing.py) 生成求解输入，[reporting.py](src/reporting.py) 生成图表与运行说明。其余模块按前述各问题的代码链接定位。
